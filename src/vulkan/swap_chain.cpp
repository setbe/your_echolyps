...
namespace hi {
    SwapChain::~SwapChain() noexcept {
        for (auto view : image_views_) vkDestroyImageView(device_.device(), view, nullptr);
        for (auto view : depth_views_) vkDestroyImageView(device_.device(), view, nullptr);
        for (auto img : depth_images_) vkDestroyImage(device_.device(), img, nullptr);
        for (auto mem : depth_memorys_) vkFreeMemory(device_.device(), mem, nullptr);
        ...
        if (render_pass_) vkDestroyRenderPass(device_.device(), render_pass_, nullptr);
    ...
    VkResult SwapChain::submit_command_buffers(const VkCommandBuffer* buffers, uint32_t* image_index) noexcept {
        ...
        constexpr size_t wait_semaphores_count{ 1 };
        constexpr size_t command_buffer_count{ 1 };
        constexpr size_t signal_semaphore_count{ 1 };
        VkSemaphore wait_semaphores[wait_semaphores_count]{ sem_image_available_[frame_index_] };
        VkPipelineStageFlags wait_stages[]{ VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT };
        VkSemaphore signal_semaphores[signal_semaphore_count]{ sem_render_finished_[frame_index_] };
    ...
    Error SwapChain::create_swapchain() noexcept {
        ...
        constexpr size_t queue_family_indices_count{ 2 };
        uint32_t queue_family_indices[queue_family_indices_count] =
    ...
    Error SwapChain::create_render_pass() noexcept { 
        ...
        VkAttachmentDescription depth_attachment{
            .format = find_depth_format(error),
    ...
    Error SwapChain::create_depth_resources() noexcept {
        ...
        depth_memorys_.resize(image_count());
        depth_views_.resize(image_count());
    ...
    Error SwapChain::create_framebuffers() noexcept { 
        ...
        for (size_t i = 0; i < image_count(); i++) {
            constexpr size_t attachments_count{ 2 };
            VkImageView attachments[attachments_count] {image_views_[i], depth_views_[i]};
    ...
    Error SwapChain::create_sync_objects() noexcept { 
        ...
        fences_images_.resize(image_count(), VK_NULL_HANDLE);
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            if (vkCreateSemaphore(device_.device(), &semaphoreInfo, nullptr, &sem_image_available_[i]) !=
                VK_SUCCESS ||
                vkCreateSemaphore(device_.device(), &semaphoreInfo, nullptr, &sem_render_finished_[i]) !=
                VK_SUCCESS ||
